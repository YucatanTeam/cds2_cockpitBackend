...
<div class="uk-margin-top" riot-view>
    ...
    <div class="uk-margin uk-text-center uk-text-muted" show="{ (Array.isArray(entries) && entries.length) || filter}">
        ...
        <img class="uk-svg-adjust" src="<?php echo $app->pathToUrl($collection['icon'] ? 'assets:app/media/icons/'.$collection['icon']:'collections:icon.svg'); ?>" width="50" alt="icon" data-uk-svg>
    ...
    <div>
        ...
        <div class="uk-width-medium-1-3 uk-viewport-height-1-2 uk-container-center uk-text-center uk-flex uk-flex-center uk-flex-middle" if="{ !loading && !entries.length && !filter }">
            ...
            <div class="uk-animation-scale">
                ...
                <?php } ?>
                <hr>
                <span class="uk-text-large"><strong><?php echo $app("i18n")->get('No entries'); ?>.</strong> <a href="<?php $app->route('/collections/entry/'.$collection['name']); ?>"><?php echo $app("i18n")->get('Create an entry'); ?>.</a></span>
        ...
        <div class="uk-clearfix uk-margin-top" show="{ !loading && (entries.length || filter) }">
            ...
            <div class="uk-float-left uk-width-1-2">
                <div class="uk-form-icon uk-form uk-width-1-1 uk-text-muted">
                    ...
                    <input class="uk-width-1-1 uk-form-large uk-form-blank" type="text" ref="txtfilter" placeholder="<?php echo $app("i18n")->get('Filter items...'); ?>" onchange="{ updatefilter }">
            ...
            <div class="uk-float-right">
                ...
                <?php if ($app->module('collections')->hasaccess($collection['name'], 'entries_edit')) { ?>
                <div class="uk-display-inline-block uk-margin-small-right" data-uk-dropdown="mode:'click'" if="{ selected.length }">
                    ...
                    <div class="uk-dropdown">
                        <ul class="uk-nav uk-nav-dropdown uk-dropdown-close">
                            ...
                            <li><a onclick="{ batchedit }"><?php echo $app("i18n")->get('Edit'); ?></a></li>
                ...
                <?php if ($app->module('collections')->hasaccess($collection['name'], 'entries_create')) { ?>
                <a class="uk-button uk-button-large uk-button-primary" href="<?php $app->route('/collections/entry/'.$collection['name']); ?>"><?php echo $app("i18n")->get('Add Entry'); ?></a>
                <?php } ?>
            </div>
        </div>
        ...
        <div class="uk-text-xlarge uk-text-muted uk-viewport-height-1-3 uk-flex uk-flex-center uk-flex-middle" if="{ !entries.length && filter && !loading }">
            <div><?php echo $app("i18n")->get('No entries found'); ?></div>
        </div>
                ...
                <div class="uk-panel uk-panel-box uk-panel-card uk-panel-card-hover">
                    ...
                    <div class="uk-position-relative uk-nbfc">
                        ...
                        <div class="uk-position-cover uk-flex uk-flex-center uk-flex-middle">
                            ...
                            <div class="uk-svg-adjust uk-text-primary" style="color:<?php echo  @$collection['color'] ; ?> !important;" if="{ !parent.isImageField(entry) }">
                                <img src="<?php echo $app->pathToUrl($collection['icon'] ? 'assets:app/media/icons/'.$collection['icon']:'collections:icon.svg'); ?>" width="80" alt="icon" data-uk-svg>
                            </div>
                        </div>
                        <a class="uk-position-cover" href="<?php $app->route('/collections/entry/'.$collection['name']); ?>/{ entry._id }"></a>
                    </div>
                        ...
                        <span data-uk-dropdown="mode:'click', pos:'bottom-right'">
                            ...
                            <div class="uk-dropdown uk-dropdown-flip">
                                <ul class="uk-nav uk-nav-dropdown">
                                    ...
                                    <?php if ($app->module('collections')->hasaccess($collection['name'], 'entries_edit')) { ?>
                                    <li><a href="<?php $app->route('/collections/entry/'.$collection['name']); ?>/{ entry._id }"><?php echo $app("i18n")->get('Edit'); ?></a></li>
                                    <?php } else { ?>
                                    <li><a href="<?php $app->route('/collections/entry/'.$collection['name']); ?>/{ entry._id }"><?php echo $app("i18n")->get('View'); ?></a></li>
                                    <?php } ?>
                                    ...
                                    <li><a class="uk-dropdown-close" onclick="{ parent.duplicateEntry }"><?php echo $app("i18n")->get('Duplicate'); ?></a></li>
                                    <?php } ?>
                                </ul>
                            </div>
                        </span>
                    ...
                    <div class="uk-margin-top uk-scrollable-box">
                        <div class="uk-margin-small-bottom" each="{field,idy in parent.fields}" if="{ field.name != '_modified' && field.name != '_created' }">
                            ...
                            <a class="uk-link-muted uk-text-small uk-display-block uk-text-truncate" href="<?php $app->route('/collections/entry/'.$collection['name']); ?>/{ parent.entry._id }">
                ...
                <tbody>
                    <tr each="{entry,idx in entries}">
                        ...
                        <td class="uk-text-truncate" each="{field,idy in parent.fields}" if="{ field.name != '_modified' && field.name != '_created' }">
                            <a class="uk-link-muted" href="<?php $app->route('/collections/entry/'.$collection['name']); ?>/{ parent.entry._id }">
                                <raw content="{ App.Utils.renderValue(field.type, parent.entry[field.name], field) }" if="{parent.entry[field.name] !== undefined}"></raw>
                                <span class="uk-icon-eye-slash uk-text-muted" if="{parent.entry[field.name] === undefined}"></span>
                            </a>
                        </td>
    ...
    <script type="view/script">
        ...
        this.fieldsidx['_created'] = {name:'_created', 'label':'<?php echo $app("i18n")->get('Created'); ?>', type: 'text'};
        this.fieldsidx['_modified'] = {name:'_modified', 'label':'<?php echo $app("i18n")->get('Modified'); ?>', type: 'text'};
